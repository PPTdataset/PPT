...
class BaseModel():
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.total_steps = 0
            epoch_iter = 0
            #print("datasize",len(self.dataloader['test'].dataset))
            ...
            tp,tn,auc = evaluate(self.gt_labels, self.an_scores, metric=self.opt.metric)
            
            performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), (self.opt.metric, auc)])
            print('Avg Run Time (ms/batch)', self.times,self.opt.metric, tp, tn ,auc)
