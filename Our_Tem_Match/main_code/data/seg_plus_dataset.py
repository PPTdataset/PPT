...
from tools.defect_generator import seamless_clone_defect_generator, seamless_clone_defect_generator_plus
...
class SegPlusDataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.dir_TC = os.path.join(opt.dataroot, 'OK_img')
    ...
    def __getitem__(self, index):
        ...
        if random.random() >= self.opt.no_defect_rate:  # 0.4
            DF_img, show_mask = seamless_clone_defect_generator_plus(self.opt, OK_img.copy(), TC_img, use_same_tc, keypoints_tc_match)
        else:
            DF_img = OK_img.copy()
