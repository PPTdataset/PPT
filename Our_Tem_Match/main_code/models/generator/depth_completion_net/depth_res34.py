...
from .net_components.residual_block import ResidualBlock
...
class _C1(nn.Module):
    def __init__(self, layer4):
        ...
        # layers
        self.block1 = layer4
        # self.block2 = ResidualBlock([2048, 256, 256, 1024], 6, 1, _NORM_LAYER)
        self.block2 = ResidualBlock([512, 64, 64, 256], 6, 1, _NORM_LAYER)
        # self.skip_block = ResidualBlock([1024, 256, 256, 1024], 6, 1, _NORM_LAYER)
        self.skip_block = ResidualBlock([256, 64, 64, 256], 6, 1, _NORM_LAYER)
...
class _C2(nn.Module):
    def __init__(self, layer3, layer4):
        super(_C2, self).__init__()
        # layers
        self.block1 = layer3
        self.block2 = _C1(layer4)
        # self.block3 = ResidualBlock([1024, 128, 128, 512], 4, 1, _NORM_LAYER)
        self.block3 = ResidualBlock([256, 32, 32, 128], 4, 1, _NORM_LAYER)
        # self.skip_block = ResidualBlock([512, 128, 128, 512], 4, 1, _NORM_LAYER)
        self.skip_block = ResidualBlock([128, 32, 32, 128], 4, 1, _NORM_LAYER)
        self.up_sample = nn.Upsample(scale_factor=2.0, mode='bilinear', align_corners=_ALIGN_CORNER)
...
class _C3(nn.Module):
    def __init__(self, layer2, layer3, layer4):
        super(_C3, self).__init__()
        # layers
        self.block1 = layer2
        self.block2 = _C2(layer3, layer4)
        # self.block3 = ResidualBlock([512, 64, 64, 256], 3, 1, _NORM_LAYER)
        self.block3 = ResidualBlock([128, 16, 16, 64], 3, 1, _NORM_LAYER)
        # self.skip_block = ResidualBlock([256, 64, 64, 256], 3, 1, _NORM_LAYER)
        self.skip_block = ResidualBlock([64, 16, 16, 64], 3, 1, _NORM_LAYER)
        self.up_sample = nn.Upsample(scale_factor=2.0, mode='bilinear', align_corners=_ALIGN_CORNER)
...
class DepthRes34(nn.Module):
    ...
    def __init__(self, in_channels, out_channels_depth, out_channels_affinity=3, pre_trained=False):
        ...
        """
        super(DepthRes34, self).__init__()

        resnet = resnet34_backbone(pre_trained)
        ...
        self.depth_block = nn.Sequential(
            BasicConv2d(64, 32, 3, padding=1, norm_layer=_NORM_LAYER),
            nn.Upsample(scale_factor=2.0, mode='bilinear', align_corners=_ALIGN_CORNER),
            nn.Conv2d(32, out_channels_depth, 3, padding=1)
        )
