...
def detection_test(model, vgg, test_dataloader, config):
    ...
    fnr = 1 - tpr
    eer_threshold1 = thresholds[np.nanargmin(np.absolute((fnr - fpr)))]
...
def localization_test(model, vgg, test_dataloader, ground_truth, config):
    ...
    if localization_method == 'gbp':
        grad = gbp_localization(model, vgg, test_dataloader,ground_truth, config)
...
def gbp_localization(model, vgg, test_dataloader, ground_true,config):
    model.eval()
    print("GBP Method:")

    grad1 = None
    i = 0
    grad1 = np.zeros((2868, 1, 128, 128), dtype=np.float32)
    for data in test_dataloader:
        X, Y = data
        ...
        for index,x in enumerate(X):
            ...
            '''save_img=np.transpose(x.numpy(),(1,2,0))*255
            save_gt=np.transpose(saliency,(1,2,0))*255
            save_gt=cv2.cvtColor(save_gt, cv2.COLOR_GRAY2BGR)
            gt=ground_true[index]
            boarder = 255 * np.ones((128, 10, 3),)
            conjunction = np.concatenate([save_img, boarder, save_gt ,boarder ,gt], 1)
            
            cv2.imwrite("val/%s.bmp" % i,conjunction)
            cv2.imwrite("result/%s.bmp" % i,save_gt)
            cv2.imwrite("gt/%s.bmp" % i,gt)'''
...
def compute_localization_auc(grad, x_ground):
    ...
    for threshold in thresholds:
        ...
        t=(grad>=threshold)
        g=(x_ground_comp>=threshold)
        if (threshold*1000)%50==0:
            miou = []
            for i in range(grad_t.shape[0]):
                inm=(t[i] & g[i])
                oum=(t[i] | g[i])
                miou.append(float(np.sum(inm))/float(np.sum(oum)))
            print(sum(miou) / len(miou),threshold)
